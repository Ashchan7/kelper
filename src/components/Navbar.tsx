...
const Navbar = ({ className = "" }: NavbarProps) => {
  ...
  return (
    <>
      ...
      {/* Mobile Menu Overlay - Fixed to cover the entire viewport */}
      <AnimatePresence>
        {isMenuOpen && (
          <>
            {/* Full screen overlay behind the menu */}
            <motion.div 
              className="fixed inset-0 w-full h-full bg-white/70 dark:bg-black/70 backdrop-blur-xl z-40"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            {/* Menu Content */}
            <motion.div
              className="md:hidden fixed inset-x-0 top-[73px] z-50 overflow-y-auto max-h-[calc(100vh-73px)]"
